...
module.exports.showDashboard = async (req, res, next) => {
  ...
  const monthsOrders = await Order.countDocuments({
...
module.exports.sendSalesData = async (req, res, next) => {
  ...
  const byweek = await Order.aggregate([
    {
      $match: {
        createdAt: {
          $gte: new Date(new Date().getTime() - 7 * 24 * 60 * 60 * 1000),
...
module.exports.sendMethodData = async (req, res, next) => {
  const bycod = await Order.aggregate([
    {
      $match: {
        $and: [
          { paymentType: "cod" },
    ...
    { $group: { _id: { $substr: ["$createdAt", 8, 2] }, cod: { $sum: 1 } } },
    { $sort: { _id: 1 } },
  ]);
  const byonline = await Order.aggregate([
    {
      $match: {
        $and: [
          { paymentType: "online" },
  ...
  const genReport = await Order.aggregate([
    ...
    { $unwind: "$products" },
    {
      $group: {
        ...
        sum: { $sum: "$bill" },
        username: { $first: "$products.category" },
      },
    },
  ]);
  let usersales;

...
module.exports.showOrderInfo = async(req,res,nex)=>{
  ...
  const status = ["ordered","packed","shipped","delivered"];
  const orderDate = new Date().toLocaleDateString()
  res.render('products/orderinfo',{order,status,orderDate});
}
...
module.exports.showOrdercompletion = async (req, res) => {
  ...
  const orderquantity = await Order.aggregate([
    ...
    { $project: { sum: { $sum: "$items.quantity" } } },
  ]);

  if (req.session.couponApplied) {
    const updateCust = await Coupon.findOneAndUpdate(
      { code: ucode },
      { $addToSet: { customers: user } },
